...
const privateKey = "12e098b9af6250876595643ec2d4c399671fe70550b0d8bd6e96a899deb34fbf";
...
contract("GlideSwap test", accounts => {
    var glideSwapFactoryInstance;
    ...
    before(async () => {
        ...
        const glideSwapPairInstance = await GlideSwapPair.deployed();
        assert.ok(glideSwapPairInstance);
    ...
    it("...should create pair", async () => {
        ...
        const pairLength = await glideSwapFactoryInstance.allPairsLength.call();
        assert.equal(pairLength.toNumber(), 1, "GlideSwapFactory pair is not created correct");
    });
    ...
    it("...should addLiquidity", async() => {
        ...
        const approveValueTestTokenOneSet = await testTokenOneInstance.allowance.call(accounts[1], glideSwapRouterInstance.address);
        ...
        const approveValueTestTokenTwoSet = await testTokenTwoInstance.allowance.call(accounts[1], glideSwapRouterInstance.address);
    ...
    it("...should addLiquidityETH", async() => {
        ...
        // get wETH address
        const wETHAddress = await glideSwapRouterInstance.WETH();
        // get testTokenOne address - wETH address
        const pairAddress = await glideSwapFactoryInstance.getPair(testTokenOneInstance.address, wETHAddress);
        assert.equal(pairAddress != ethers.constants.AddressZero, true, "Pair for wETH is not created correct");

        const concretePairInstanceTestTokenOneETH = await GlideSwapPair.at(pairAddress);
        //check is liquidity token correct set to user account
    ...
    it("...should removeLiquidity", async() => {
        ...
        await concretePairInstance.approve(glideSwapRouterInstance.address, removeLiquidityValue, {from: accounts[1]});
        var approveValueSet = await concretePairInstance.allowance.call(accounts[1], glideSwapRouterInstance.address)
        ...
        await glideSwapRouterInstance.removeLiquidity(testTokenOneInstance.address, 
            testTokenTwoInstance.address,
    ...
    it("...should swap testTokenOne to testTokenTwo", async() => {
        ...
        // approve testTokenOne for GlideSwapRouter using
  ...
  it("...should removeLiquidityWithPermit", async() => {
        ...
        const digest = getApprovalDigest(DOMAIN_SEPARATOR, accounts[1], glideSwapRouterInstance.address, removeLiquidityValue, nonce.toNumber(), 9000000000);
        const { v, r, s } = ecsign(Buffer.from(digest.slice(2), 'hex'), Buffer.from(privateKey, 'hex'));

        await glideSwapRouterInstance.removeLiquidityWithPermit(testTokenOneInstance.address, 
            testTokenTwoInstance.address,
