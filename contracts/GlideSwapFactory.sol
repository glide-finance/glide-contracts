...
import './GlideSwapPair.sol';

contract GlideSwapFactory is IGlideSwapFactory {
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(abi.encodePacked(type(GlideSwapPair).creationCode));
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        bytes memory bytecode = type(GlideSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IGlideSwapPair(pair).initialize(token0, token1);
    ...
    function getReserves(address tokenA, address tokenB) public view override returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IGlideSwapPair(pairFor(tokenA, tokenB)).getReserves();
